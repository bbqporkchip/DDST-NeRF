...
def raw2outputs(raw_s,
    ...
    T_full = torch.cumprod(torch.cat(
        [torch.ones((alpha_d.shape[0], 1)), (1. - alpha_d * blending) * (1. - alpha_s * (1. - blending)) + 1e-10], -1),
                           -1)[:, :-1]
...
def render_rays(t,
    ...
    raw_s = network_query_fn_s(pts_ref[..., :3], viewdirs, network_fn_s, f_projection)
    ...
    raw_d = network_query_fn_d(pts_ref, viewdirs, network_fn_d, f_projection)
    ...
    ret = {'rgb_map_full': rgb_map_full, 'depth_map_full': depth_map_full, 'acc_map_full': acc_map_full,
    ...
    raw_d_b = network_query_fn_d(pts_b, viewdirs, network_fn_d, pts_b_feature)
    ...
    with torch.no_grad():
        ...
        raw_surf = network_query_fn_d(surf_ref, viewdirs, network_fn_d, f_projection)
    ...
    with torch.no_grad():
        pts_b_feature = ref.feature_matching(pts_b[..., :3])
    raw_d_f = network_query_fn_d(pts_f, viewdirs, network_fn_d, pts_b_feature)
    ...
    ret['surf_f'] = surf_f
    # ret['raw_d_f_hat'] = torch.sum(weights_d_f[..., None] * pts_f[..., :3, -2])
    ret['raw_d_f_hat'] = torch.sum(weights_d_f[..., None] * pts_f[..., :3], dim=-1)  # 检查形状后，按需修改
    ...
    if chain_5frames:
        ...
        raw_d_b_b = network_query_fn_d(pts_b_b, viewdirs, network_fn_d,pts_b_b_feature)
    ...
    if chain_5frames:
        ...
        raw_d_f_f = network_query_fn_d(pts_f_f, viewdirs, network_fn_d, pts_f_f_feature)
