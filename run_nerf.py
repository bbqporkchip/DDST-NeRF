...
# os.environ["CUDA_VISIBLE_DEVICES"] = "0.1"    #c
...
def config_parser():
    ...
    parser.add_argument('--config', is_config_file=True, default="./configs/config_Balloon1.txt",
                        help='config file path')  #需要修改config的路径
    ...
    parser.add_argument("--lrate", type=float, default=5e-4,
                        help='learning rate')
    parser.add_argument("--d_lrate", type=float, default=5e-4,   #c
                        help='learning rate')                       #c
    parser.add_argument('--depth_loss_lambda', type=float, default=0.04, help='Depth loss coefficient') #d
    parser.add_argument('--flow_loss_lambda', type=float, default=0.02, help='Flow loss coefficient')#d
    ...
    # parser.add_argument("--depth_loss_lambda", type=float, default=0.04,   #c
    #                     help='lambda of depth loss')                        #c
    parser.add_argument("--order_loss_lambda", type=float, default=0.1,
                        help='lambda of order loss')
    # parser.add_argument("--flow_loss_lambda", type=float, default=0.02,         #c
    #                     help='lambda of optical flow loss')                 #c
    ...
    parser.add_argument("--surface_loss_lambda", type=float, default=0.01,   #d
                        help='lambda of surface loss')                      #d
...
def train():
    ...
    # Create nerf model
    # encoder = EncoderNetwork(args)  # 假设 EncoderNetwork 是自定义的编码器类
    # render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer = create_nerf(args)
    render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer, encoder = create_nerf(args)
    # encoder = SomeEncoderClass()
    # render_kwargs_train, render_kwargs_test, start, grad_vars, optimizer= create_nerf(args)
    # encoder = SomeEncoderClass()  # 初始化 encoder
    global_step = start
    ...
    if args.render_only:
        ...
        pose2render = torch.Tensor(poses[0:1, ...]).expand([int(num_img), 3, 4])                                #c
        time2render = i_train / float(num_img) * 2. - 1.0
        result_type = 'testset_view000'

        testsavedir = os.path.join(
            basedir, expname, result_type + '_{:06d}'.format(i))                                                #c
        os.makedirs(testsavedir, exist_ok=True)
        with torch.no_grad():
            ret = render_path(pose2render, time2render,
                              hwf, args.chunk, render_kwargs_test, savedir=testsavedir, ref=ref)
        moviebase = os.path.join(                                                                                   #c
            testsavedir, '{}_{}_{:06d}_'.format(expname, result_type, i))                                   #c
        save_res(moviebase, ret)
            ...
            for i in range(args.N_iters):
                ...
                if i % args.i_img == 0:
                    ...
                    with torch.no_grad():
                        ...
                        writer.add_image("rgb_s", torch.clamp(ret['rgb_map_s'], 0., 1.), global_step=i,
                                         dataformats='HWC')
                        writer.add_image("depth_s", normalize_depth(ret['depth_map_s']), global_step=i,
                                         dataformats='HW')
    ...
    for i in range(start, args.N_iters):
        ...
        warp_loss_1, warp_1 = warp_comparsion(ret_curr['rgb_map_d'], ret_rand['rgb_map_d'], ret_curr['depth_map_d'],
                                              pose, pose_rand, int(patch_s), int(patch_s), focal)
        warp_loss_2, warp_2 = warp_comparsion(ret_curr['rgb_map_full'], ret_rand['rgb_map_full'],
                                              ret_curr['depth_map_full'], pose, pose_rand, int(patch_s), int(patch_s),
                                              focal)
        ...
        if i % args.i_img == 0:
            ...
            with torch.no_grad():
                ...
                imageio.imwrite(os.path.join(testimgdir, '{:06d}.png'.format(i)),
                                to8b(ret['rgb_map_full'].cpu().numpy()))

                writer.add_scalar("psnr_holdout", psnr.item(), i)
                writer.add_image("rgb_holdout", target, global_step=i, dataformats='HWC')
                writer.add_image("mask", mask, global_step=i, dataformats='HW')
                writer.add_image("disp", torch.clamp(invdepth / percentile(invdepth, 97), 0., 1.), global_step=i,
                                 dataformats='HW')
                ...
                writer.add_image("patch_rgb_rand",
                                 torch.clamp(ret_rand['rgb_map_d'].reshape(int(patch_s), int(patch_s), 3), 0., 1.),
                                 global_step=i, dataformats='HWC')
                writer.add_image("patch_rgb_rand",
                                 torch.clamp(ret_rand['rgb_map_full'].reshape(int(patch_s), int(patch_s), 3), 0., 1.),
                                 global_step=i, dataformats='HWC')
                writer.add_image("patch_rgb_curr",
                                 torch.clamp(ret_curr['rgb_map_d'].reshape(int(patch_s), int(patch_s), 3), 0., 1.),
                                 global_step=i, dataformats='HWC')
                writer.add_image("patch_rgb_curr",
                                 torch.clamp(ret_curr['rgb_map_full'].reshape(int(patch_s), int(patch_s), 3), 0., 1.),
                                 global_step=i, dataformats='HWC')

                writer.add_image("warp_d", torch.clamp(warp_1[0].permute(1, 2, 0), 0., 1.), global_step=i,
                                 dataformats='HWC')
                writer.add_image("warp_f", torch.clamp(warp_2[0].permute(1, 2, 0), 0., 1.), global_step=i,
                                 dataformats='HWC')
